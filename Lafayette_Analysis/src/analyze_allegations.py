...
def clean_allegation_text(allegation):
    """
    Clean and standardize allegation text for consistent categorization
    """
    ...
    standard_categories = {
        # Use of Force variations
        'use_of_force': [
            'use of force', 'use of force/courtesy', 'use of force/performance of duty',
            'use of force/reports', 'unlawful harassment/use of force',
            'courtesy/use of force', 'courtesy/performance of duty/use of force',
            'performance of duty/use of force'
        ],

        # Performance of Duty variations
        'performance_of_duty': [
            'performance of duty', 'performance of duty/courtesy',
            'performance of duty/request for assistance', 'courtesy/performance of duty', 'unsatisfactory performance', 'courtesy/unsatisfactory performance',
            'identification/unsatisfactory performance'
        ],

        # Conduct Unbecoming variations
        'conduct_unbecoming': [
            'conduct unbecoming', 'associations/conduct unbecoming', 'courtesy', 'courtesy/identification', 'courtesy/performance of duty',
            'courtesy/unlawful harassment', 'courtesy/unsatisfactory performance',
            'unlawful harassment/courtesy', 'performance of duty/courtesy'
        ],

        # Arrest/Search/Seizure variations
        'arrest_search_seizure': [
            'arrest search and seizure', 'arrest search and seizure ', 'arrest',
            'illegal search'
        ],

        # Treatment of persons in custody variations
        'treatment_custody': [
            'treatment of persons in custody', 'treatment of prisoners in custody',
            'treatment of prisoners', 'treatment of persons in custody prea unfounded'
        ],

        # Unlawful Harassment variations
        'unlawful_harassment': [
            'unlawful harassment', 'unlawful harassment/courtesy',
            'courtesy/unlawful harassment'
        ],

        # Dissemination of Information
        'information_dissemination': ['dissemination of information'],

        # Sexual misconduct
        'sexual_misconduct': ['prea','sexual harassment', 'sexual contact prohibited'],

        # Associations/Off-duty conduct
        'associations_off_duty_conduct': ['associations', 'police action off duty', 'police action off-duty'],

        # Policy violations
        'policy_violations': [
            'body worn camera', 'social networking', 'e-mail policy', 'vehicle use',
            'outside employment', 'departmental reports'
        ],

        # Criminal conduct
        'criminal_conduct': [
            'accused charged or convicted', 'accused or convicted of a crime',
            'intoxicants', 'payment of debts'
        ],

        # Authority/Badge abuse
        'abuse_authority': [
            'abuse of authority', 'badge of office', 'intervention in a civil matter',
            'intervention in civil matter'
        ],

        # Bias/Profiling
        'bias_profiling': ['bias-based profiling'],

        # Administrative
        'administrative': [
            'identification', 'employee searches', 'attendance', 'firearm discharge',
            'operation of vehicle', 'traffic enforcement', 'endorsement referrals',
            'relations with other members', 'involvement in altercation',
            'interference', 'conformance of law', 'conformance of laws'
        ]
    }
...
def perform_so_regression_analysis(df, allegation_col='allegation_clean', min_cases=5):
    """
    Perform logistic regression analysis for Lafayette SO data
    """
    print("\n" + "="*60)
...
def create_so_regression_visualizations(regression_results, output_file):
    """
    Create visualizations for Lafayette SO regression analysis results
    """
    ...
    bars = ax1.barh(y_pos, sustain_df_sorted['sustain_rate'],
                   color=['red' if rate > 35 else 'orange' if rate > 20 else 'green'
    ...
    ax3 = axes[1, 0]
    ax3.hist(sustain_df['sustain_rate'], bins=8, alpha=0.7, color='lightcoral', edgecolor='black')
    ax3.axvline(sustain_df['sustain_rate'].mean(), color='darkred', linestyle='--',
...
def analyze_lafayette_allegations():
    """
    Analyze allegations in Lafayette SO complaint data
    """

    # Read the data
    print("Loading Lafayette SO complaint data...")
    df = pd.read_csv('../data/cprr_lafayette_so_2006_2020.csv')

    print(f"Total records: {len(df)}")
    print(f"Date range: 2006-2020")
    ...
    df.loc[df['disposition_clean'] == 'non-sustained', 'disposition_clean'] = 'not sustained'

    # Create output directory
    os.makedirs('../visualizations', exist_ok=True)

    print("\n" + "="*60)
    print("LAFAYETTE SO COMPLAINT ANALYSIS (2006-2020)")
    print("="*60)
    ...
    regression_results = perform_so_regression_analysis(df)

    # Create visualizations
    create_allegation_visualizations(df, allegation_counts, sustained_counts,
                                   disposition_counts, sustain_rates)

    # Create regression visualizations
    create_so_regression_visualizations(regression_results,
                                      '../visualizations/lafayette_so_regression_analysis.png')

    print(f"\nVisualizations saved to ../visualizations/")
    print("- lafayette_so_allegations_analysis.png")
    print("- lafayette_so_regression_analysis.png")

def create_allegation_visualizations(df, allegation_counts, sustained_counts,
                                   disposition_counts, sustain_rates):
    """Create comprehensive visualizations for allegation analysis"""
    ...
    plt.title('Top 10 Sustained\nComplaint Types', fontweight='bold')
    plt.gca().invert_yaxis()
    ...
    colors_rates = ['red' if rate > 30 else 'orange' if rate > 15 else 'green'
    ...
    plt.title('Sustain Rates by\nComplaint Type', fontweight='bold')
    plt.gca().invert_yaxis()
    ...
    # Extract year from receive_date or receive_year
    df['year'] = df['receive_year'].fillna(0).astype(int)
    df.loc[df['year'] == 0, 'year'] = pd.to_datetime(df['receive_date'], errors='coerce').dt.year

    yearly_complaints = df.groupby('year').size()
    yearly_sustained = df[df['disposition_clean'] == 'sustained'].groupby('year').size()

    valid_years = yearly_complaints.index[(yearly_complaints.index >= 2006) &
                                         (yearly_complaints.index <= 2020)]
    ...
    plt.title('Complaint Trends\nOver Time', fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)

    # 6. Department breakdown
    plt.subplot(2, 4, 6)
    dept_counts = df['department_desc'].value_counts()
    colors_dept = ['lightcoral', 'lightblue', 'lightgreen'][:len(dept_counts)]
...
• Not Sustained: {disposition_counts.get('not sustained', 0)}
    ...
    plt.tight_layout()
    plt.savefig('../visualizations/lafayette_so_allegations_analysis.png',
